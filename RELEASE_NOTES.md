...
## Export to Excel on every grid

- Every grid in the portal now carries an **Export to Excel** button, next to its search box. It produces a UTF-8 CSV (with a byte-order mark, so Excel picks the encoding up without an import wizard) that opens straight into Excel on a double-click.
- **The export follows what is on screen, not the page you are on.** It reuses the same predicate the grid itself uses to decide which rows to show, so the global search box, every per-column filter, the server and severity filters on assessment reports and the Yes / Close / No tiles on SQL QA Compare all narrow the file. Paging does not: showing 10 entries and exporting still gives you all 400 matching rows.
- **Action columns are left out.** A column is excluded when its header is blank, reads "Actions", holds a tick box or dropdown, or is marked `data-export="skip"`. Everything else is exported as the text you see, so a Critical badge exports as `Critical` and a database-count button as its number. Values containing commas, quotes or line breaks are quoted and escaped.
- **File name** comes from `data-export-name` on the table when set, otherwise from the nearest heading above the grid, plus the date - for example `Servers - 20260922.csv`. A grid can opt out entirely with `data-no-export`.
- Implemented once in `wwwroot/js/site.js`, inside the shared `attachTable` toolbar, so no view needed changing and any grid added later gets the button for free.
- The bespoke **Export CSV** button on SQL QA Compare has been removed in favour of this one. The replacement keeps the old `QA Report - <server1>__<server2>` file name and additionally honours the column filters and the match tiles, which the old button ignored.

## Findings are ranked against the server's environment

- A dev box with no backups was raising the same **Critical** as production, so the Summary tiles counted things nobody would get out of bed for. Findings are now capped by the environment of the server they were raised on: **UAT and Staging cap at High, Development and Test cap at Medium**, Production and DR are untouched.
- **Nothing on a Development or Test server can be reported as Critical** - not even a security or encryption finding. That tile is reserved for things nobody can defer, and a dev box is not one of them.
- **Security and Encryption are otherwise left alone.** A foothold on a dev box on the same domain is still a foothold, and non-production frequently holds a copy of production data, so on Production and UAT they keep whatever the collector raised. On Development they land at **High** rather than being flattened to Medium with the operational findings, so a security issue there still outranks the routine dev noise around it.
- **Licensing moves both ways rather than only down.** Developer edition in production is a licensing breach and keeps its severity; the same edition in Development drops to Info, because that is exactly what is supposed to be there.
- **An unrecognised or blank environment is treated as Production.** This is the safety of the whole feature - a production server with a blank or misspelled Environment keeps its Criticals. A hidden emergency is worse than the noise this removes, because noise is at least visible.
- **Environment is now a controlled dropdown** on the server form, validated server-side against the same list the policy uses. It decides whether a finding may be reported as Critical, so it can no longer be free text where a typo would silently downgrade a production server. The register's filter still offers existing values so current rows stay findable while they are corrected.
- **The original severity is kept.** New `AssessmentFindings.BaseSeverity` holds what the collector raised, and the Findings grid shows "from Critical" under a capped row. Without that, a genuine Medium and a re-ranked Critical would be indistinguishable and the rule could never be audited or tuned.
- **Counts come from the capped findings**, not the collector's `ExecutiveSummary` totals, so the Summary tiles and the Findings tab agree.
- **Existing runs are re-ranked at startup** (`Data/FindingSeverityBackfill.cs`), so historical assessments match new ones. The pass always works from `BaseSeverity`, so it is idempotent and can never walk a finding progressively down the scale; it also means correcting a server's Environment in the register re-ranks its past assessments on the next restart.

## Sysadmins tab split by account class

- The Sysadmins tab listed every member of the sysadmin role in one grid. SQL Server setup puts its own service SIDs into that role on every instance, so on a fourteen-server estate those rows are the large majority and they bury the handful a security review actually cares about. The tab is now five sub-tabs, each with its count: **Windows users**, **Windows groups**, **SQL logins**, **Unrecognised service accounts**, **Built-in service accounts**, opening on Windows users.
- **Nothing is dropped.** The built-ins are classified, not filtered out, for two reasons: their *absence* is the finding (no `NT SERVICE\SQLSERVERAGENT` means Agent jobs will misbehave), and an auditor asking for every sysadmin gets the complete list without a re-run.
- **The split that matters is built-in vs unrecognised.** A blanket "hide `NT SERVICE\*`" rule would also have hidden accounts setup never created - the Azure SQL IaaS Agent extension's `NT Service\SQLIaaSExtensionQuery`, third-party backup and monitoring agents. Those are software with full control of the instance and now have a tab of their own.
- New `Services/SysadminClassifier.cs`. The built-in list is matched as a **prefix**, so a named instance's `NT SERVICE\MSSQL$PAYROLL` and `NT SERVICE\SQLAgent$PAYROLL` are recognised rather than reported as unexpected. A principal is only treated as a service account when it is also a Windows principal, so a SQL login someone has named `NT SERVICE\...` is not mistaken for the real thing.
- Classification uses `type_desc`, which the collector already returns, so existing runs reclassify without a new assessment.
- Each sub-tab exports separately (`Sysadmins - Windows users.csv` and so on).

## Certificates tab split in two

- The Certificates tab stacked **Database certificates** above **TLS / connection certificates**, so the second grid was off the bottom of a long first one. They are now two sub-tabs within the Certificates tab, each showing its row count, with Database certificates open by default.
- Rendered as pills rather than a second row of tabs, so a tab strip inside a tab strip still reads as a level down.
- **Database certificates column order**: Starts, Expires and Days to expiry moved up to sit directly after Server, ahead of Certificate / Subject / Issuer, and the three date columns no longer wrap - "Expired 4142 d ago" was breaking across two lines and doubling every row's height.
- **Each grid on the assessment report now exports under its own name.** Only the certificates section had a heading above its table, so every other tab fell back to the page title and all thirteen exported as the same file. All 15 tables now carry an explicit `data-export-name` (`Findings`, `Volumes`, `SQL Agent jobs`, ...).

## Grids default to 100 entries

- Every grid now opens showing **100 entries** instead of 25 (registers) or 10 (assessment and compare tabs). The Show dropdown still offers 10 / 25 / 50 / 100.
- The Servers, Databases, Applications and Costs registers each declared their own copy of the toolbar markup, so the default lived in five places. They now share `Views/Shared/_TableToolbar.cshtml`, and `site.js` builds the identical toolbar from `PAGE_SIZE_DEFAULT` for grids that do not declare one - two places instead of five, and any grid added later inherits the default.
- Export is unaffected: it has never been bound to the page size, so it still writes every row matching the filters.
